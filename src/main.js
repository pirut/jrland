...
function getNetConfigFromUrl() {
  ...
  return {
    ...
    token: params.get("token") ?? getStoredToken(),
  };
}

function updateAuthUI() {
  if (!authStatus || !authSignIn || !authSignOut) return;
  const token = getStoredToken();
  const profile = getProfileFromToken(token);
  if (token) {
    const label = profile?.email || profile?.preferred_username || profile?.sub || "Signed in";
    authStatus.textContent = `Signed in as ${label}`;
    authSignIn.style.display = "none";
    authSignOut.style.display = "inline-flex";
  } else {
    authStatus.textContent = "Sign in to save your account.";
...
async function bootstrap() {
  if (isAuthCallback()) {
    if (authStatus) authStatus.textContent = "Signing you in...";
  ...
  if (netAdapter?.connect && netConfig.token) {
    netAdapter.connect();
  }
...
bootstrap().catch((error) => {
  console.error(error);
  if (authStatus) authStatus.textContent = error?.message ?? "Startup failed";
});
